...
class Controller(object):
    ...
    def setup(cls):
        ...
        Server.Process.get().add_url_rule('/base/login/', 'base.login', controllers.web.login.local.index,
                                          methods=['GET'])
        Server.Process.get().add_url_rule('/base/login/', 'base.login.send', controllers.web.login.local.login,
                                          methods=['POST'])
        Server.Process.get().add_url_rule('/base/logout/', 'base.logout', controllers.web.login.local.logout,
                                          methods=['POST'])
        try:
            Server.Process.get().add_url_rule('/logout/', 'logout', controllers.web.login.local.logout,
                                              methods=['POST'])
            Server.Process.get().add_url_rule('/login/', 'login', controllers.web.login.local.index, methods=['GET'])
            Server.Process.get().add_url_rule('/login/', 'base.login.send', controllers.web.login.local.login,
                                              methods=['POST'])
